...
class Cash_game(Salon):  #à  faire
    ...
    def write_data(self, name, password):
        self.fichier_data=self.fichier_data.append({"name":name, "stack":str(self.stack), "password":password}, ignore_index=True)
        self.fichier_data.to_csv(r"c:\Users\VIE ShareWizMe\Desktop\CPES L2\Algo\Projet poker\Poker\server\data.csv", index=False)
    ...
    def minute_check(self): 
        ...
        if len(self.players)>1:  # ajouter une condition pour que les joueurs soit pret
            ...
            while give_chaises_dispo(repartit_tables, self.n_max)>0 and current_wait_file:
                ...
                repartit_tables=repartit_tables=[len(table) for table in self.tables] # prend en compte le joueur ajouté

            """créer une nouvelle table si assez de nouveaux joueurs"""
            while len(current_wait_file) > give_chaises_dispo(repartit_tables, self.n_max) and len(current_wait_file)>1:
