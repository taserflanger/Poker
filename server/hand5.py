...
class Hand5:
    ...
    def get_type(self):
        ...
        return max(attributes, key=lambda attr: Hand5.types_ranking.index(attr)), top_comb_values, low_comb_values
    ...
    def c_same_type(self, hand2):
        ...
        for i in range(len(self.top_comb_values)):
            if self.top_comb_values[i] != hand2.top_comb_values[i]:
                return self.top_comb_values > hand2.top_comb_values, False
        for i in range(len(self.low_comb_values)):
            if self.low_comb_values[i] != hand2.low_comb_values[i]:
                return self.low_comb_values > hand2.low_comb_values, False
    ...
    def __gt__(self, other_hand):
        if other_hand is None:  # utile dans rank_hands: les joueurs couchés ont la main None
            return True
        type_ids = (Hand5.types_ranking.index(self.type), Hand5.types_ranking.index(other_hand.type))
        if type_ids[0] > type_ids[1]:
            return True
        elif type_ids[0] == type_ids[1]:
            return Hand5.c_same_type(self, other_hand)[0]
        else:
            return False
    ...
    def __eq__(self, other_hand):
        ...
        type_ids = (Hand5.types_ranking.index(self.type), Hand5.types_ranking.index(other_hand.type))
        if type_ids[0] == type_ids[1]:
            return Hand5.c_same_type(self, other_hand)[1]
        else:
            return False
...
def test_class(nb_players=6):
    ...
    players_cards = [[deck.pop() for _ in range(2)] for _ in range(nb_players)]
    ...
    for i in range(nb_players):
        ...
        possible_hands = [Hand5(i) for i in combinations(table_cards + players_cards[i], 5)]
        best_hand = max(possible_hands)
        players_fh[i] = best_hand
    ranked_hands = utils.rank_dict(players_fh)
